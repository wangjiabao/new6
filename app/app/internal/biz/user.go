...
func (uuc *UserUseCase) VipCheck(ctx context.Context, req *v1.VipCheckRequest) (*v1.VipCheckReply, error) {
	...
	for _, user := range users {
		...
		if teamCsdBalance >= vip5BalanceTeam && 2 <= vip4Count && 2 <= user.HistoryRecommend && myUserBalance >= vip5Balance {
			...
			myVip = 5
		} else if teamCsdBalance >= vip3BalanceTeam && 2 <= vip2Count && 2 <= user.HistoryRecommend && myUserBalance >= vip3Balance {
			myVip = 4
		} else if teamCsdBalance >= vip2BalanceTeam && 2 <= vip1Count && 2 <= user.HistoryRecommend && myUserBalance >= vip2Balance {
			myVip = 3
